...
public:
NodoArbol(string clave, int valor) : clave(clave), valor(valor), izq(0), der() {
// constructor vacio
}
};
...
void ArbolBinarioBusqueda::auxiliarEnOrden (Nodo Arbol *raizSubarbol, vector<string> &v) {
if (raizSubarbol == 0)
return;
auxiliarEnOrden(raizSubarbol->izq, v);
v.push_back(raizSubarbol->clave);
auxiliarEnOrden(raiz Subarbol->der, v);
}
...
if (raiz Subarbol == 0)								// la clave no existe, no hacer nada
return;
if (clave < raiz Subarbol->clave)					// buscar la clave
this->auxiliarBorrar(raizSubarbol->iza, clave);
else if (clave > raizSubarbol->clave)
